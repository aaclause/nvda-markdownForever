...
##  2. But what on Earth this Markdown thing is?
...
mistakes, you will use simple tags and your work will be done in no time. With
Markdown, you will be able to do more or less the things HTML can do:
headings, ordered (numbered) or unordered (buleted) lists, links, and even
tables.

For example, if you want to make a first level heading, you will write one
hash sign (#), followed by the name of your heading:
...
are numerous. Just check this [ Markdown Cheat Sheet
](https://www.markdownguide.org/cheat-sheet/) for more tags and details.
...
the virtual buffer of NVDA. Some tags such as ` <details> ` are not supported
by the render of virtual buffer.

##  4. Converting Markdown to HtML source code
...
properly set as the default one in  MarkdownForever's settings,  located in
the Settings sub menu of NVDA.

##  6. Converting Markdown to Formatted HtML
...
document, independent of the  default settings.  This must be placed at the
very beginning of the document and must begin with three dashes ("---") and
end with three periods ("...") or three dashes ("---"). Each entry must be
...
###  8.1. List of supported metadata
...
Extra-tags are special placeholders that you put in your Markdown content to
automatically add system linfo like current date or time, which will be
...
---|---|---  
` %date% ` |  Current date  |  11/1/2019  
` %time% ` |  Current time  |  8:30:35 PM  
` %now% ` |  Current date / time  |  11/1/2019 8:30:35 PM  
` %day% ` |  Current day (long format)  |  Friday  
` %dday% ` |  Current day (decimal format)  |  01  
` %month% ` |  Month (long format)  |  November  
...
##  10. MarkdownForever's default settings
