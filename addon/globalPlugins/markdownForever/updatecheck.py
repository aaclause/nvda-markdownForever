...
def checkUpdates(sil=False):
	...
	def errorUpdateDialog(details=None):
		global checkInProgress
		checkInProgress = True
		msg = _("Oops! There was a problem downloading Markdown Forever update. Please retry later or download and install manually from %s. Do you want to open this URL in your browser?") % addonInfos["url"]
		if details:
	...
	title = _("Markdown Forever update")
	newUpdate = False
	url = addonInfos["url"]
	if url.endswith('/'):
		url = url[0:-1]
