...
* "Разрешить обратный перевод экстратегов": Указывает, следует ли возвращать [extra-tags](#extra-tags) к их первоначальному виду (например, %date%) при преобразовании HTML-контента, сгенерированного Markdown Forever, обратно в Markdown. 
...
Экстра-теги - это специальные заполнители, которые вы можете поместить в содержимое Markdown для автоматического добавления системной информации, например, текущей даты или времени, которые будут отображаться в HTML-документе открытым текстом.

### Поддерживаемые экстра-теги
