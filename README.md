...
As usual, this works on the entire text displayed on the screen or on a specificly selected part. The following commands are available:
...
However, you must first ensure that the HTML2Text conversion engine is properly set as the default one in MarkdownForever's settings, located in the Settings sub menu of NVDA.
...
The interactive mode is a function which presents all MarkdownForever's features in a convenient dialog box, which offers you even more handy extras. To open the Interactive Mode, Press NVDA+CTRL+i. Here is a description of the screen in Tab order:
  ...
  * "Allow Extratags back translation": This will indicate whether or not extra-tags should be reverted to their original form (e.g. 12/9/2019) when converting an HTML content generated by Markdown Forever back to Markdown. 
...
### List of supported metadata
  ...
  * extratags-back: To indicate whether or not extra-tags should be reverted to their original form (e.g. 12/9/2019) when converting an HTML content generated by Markdown Forever back to Markdown. Possible values: true (1) or false (0).
